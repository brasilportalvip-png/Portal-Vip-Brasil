...
export interface ScheduledPost { id:string; userId:string; companyId:string; contentItemId:string; platforms:string[]; scheduledFor:string; status:'scheduled'|'publishing'|'published'|'failed'|'cancelled'|'requires_review'|'planned'; isPlanning?:boolean; publishedAt?:string; errorMessage?:string; autopilotGenerated?:boolean; retryCount?:number; providerOptions?:{pinterestBoardId?:string;youtubePrivacyStatus?:'private'|'unlisted'|'public'}; publicationResults?:Array<{platform:string;provider?:string;success:boolean;externalId?:string|null;error?:string;externalState?:'confirmed_success'|'confirmed_failed'|'unknown';retrySafe?:boolean;requiresUserAction?:boolean;deliveryMode?:'published'|'draft';statusCode?:number}>; createdAt:string; updatedAt?:string; }
...
export interface VideoJob {
  ...
  status: 'queued' | 'pending' | 'processing' | 'retry_scheduled' | 'rendering' | 'ready_to_publish' | 'publishing' | 'published' | 'finalizing' | 'completed' | 'failed' | 'failed_permanent';
  progressPct: number;
  videoUrl?: string;
  storagePath?: string;
