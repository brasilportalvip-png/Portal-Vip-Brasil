...
export const ProfilePage: React.FC<Props> = ({ user, onRefreshUser }) => {
  ...
  return <div className="mx-auto max-w-4xl space-y-6 animate-fadeIn">
    ...
    <form onSubmit={submit} className="space-y-6">
      <section className="froc-panel space-y-4"><h3 className="froc-section-title">Informações pessoais</h3><div className="grid gap-4 md:grid-cols-2"><label className="text-xs font-semibold text-slate-300">Nome completo<input value={name} onChange={(e)=>setName(e.target.value)} autoComplete="name" className="froc-input mt-1.5" required/></label><label className="text-xs font-semibold text-slate-300">E-mail<input value={user?.email||''} type="email" disabled className="froc-input mt-1.5 opacity-60"/></label></div><div className="flex flex-wrap gap-4 text-xs text-slate-400"><span className="flex items-center gap-1"><Calendar size={13} className="text-cyan-400"/>Membro desde {user?.createdAt?new Date(user.createdAt).toLocaleDateString('pt-BR'):'—'}</span><span className="flex items-center gap-1"><ShieldCheck size={13} className="text-emerald-400"/>Conta protegida pelo Firebase Auth</span><span className="flex items-center gap-1"><ShieldCheck size={13} className="text-cyan-400"/>Central administrativa privada</span></div></section>
