// src/lib/nicheVideoCatalog.ts
// Catálogo Estratégico de Vídeos e Roteiros Virais por Nicho de Aplicativo (TikTok / Reels / Shorts)
// Otimizado para alta retenção (0-3s), tráfego orgânico e conversão de downloads na Google Play Store.

export interface NicheVideoScene {
  sceneNumber: number;
  timeSeconds: string;
...
export interface NicheVideoTemplate {
  id: string;
  nicheName: string;
...
export const NICHE_VIDEO_TEMPLATES: Record<string, NicheVideoTemplate> = {
  runes_viking: {
    id: 'runes_viking',
...
/**
 * Detecta o nicho mais apropriado para vídeos e roteiros com base no slug do projeto,
 * título, tema ou palavras-chave.
 */
export function detectNicheForVideo(text?: string, projectSlugOrId?: string): NicheVideoTemplate {
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.runes_viking;
  }
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.love_maria_padilha;
  }
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.exu_guardioes;
  }
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.catolico_devocao;
  }
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.prosperidade_crencas;
  }
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.tarot_oraculos;
  }
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.ia_inteligencia_artificial;
  }
  ...
  if (
    ...
    return NICHE_VIDEO_TEMPLATES.trafego_organico_growth;
  }

  // Se o projectSlug bate diretamente
  if (projectSlugOrId) {
    const slug = projectSlugOrId.toLowerCase();
    if (slug.includes('exu')) return NICHE_VIDEO_TEMPLATES.exu_guardioes;
    if (slug.includes('padilha')) return NICHE_VIDEO_TEMPLATES.love_maria_padilha;
    if (slug.includes('catolico')) return NICHE_VIDEO_TEMPLATES.catolico_devocao;
    if (slug.includes('crenca') || slug.includes('crença')) return NICHE_VIDEO_TEMPLATES.prosperidade_crencas;
    if (slug.includes('oraculo')) return NICHE_VIDEO_TEMPLATES.tarot_oraculos;
    if (slug.includes('froc')) return NICHE_VIDEO_TEMPLATES.ia_inteligencia_artificial;
  }

  // Padrão: Tráfego orgânico & crescimento de apps
  return NICHE_VIDEO_TEMPLATES.trafego_organico_growth;
}
